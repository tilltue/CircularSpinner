...
extension ViewController: CircularSpinnerDelegate {
    ...
    func circularSpinnerTitleForValue(_ value: Float) -> NSAttributedString {
        ...
        if #available(iOS 8.2, *) {
            attributeStr.addAttributes([.font: UIFont.systemFont(ofSize: 60, weight: .thin)], range: NSMakeRange(0, attributeStr.string.count - 1))
            attributeStr.addAttributes([.font: UIFont.systemFont(ofSize: 40, weight: .ultraLight)], range: NSMakeRange(attributeStr.string.count - 1, 1))
        } else {
            attributeStr.addAttributes([.font: UIFont.systemFont(ofSize: 60)], range: NSMakeRange(0, attributeStr.string.count - 1))
            attributeStr.addAttributes([.font: UIFont.systemFont(ofSize: 40)], range: NSMakeRange(attributeStr.string.count - 1, 1))
        }
        return attributeStr
    }
}
